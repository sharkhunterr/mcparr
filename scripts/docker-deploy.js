#!/usr/bin/env node
/**
 * Docker Hub deployment script for MCParr
 *
 * Features:
...
function getDockerConfig() {
  const pkg = JSON.parse(fs.readFileSync('package.json', 'utf8'));
  return {
    image: pkg.config?.dockerImage || 'sharkhunterr/mcparr',
...
function main() {
  console.log('🐳 MCParr Docker Deployment Script\n');
  ...
  if (options.multiPlatform) {
    ...
    exec(
      'docker buildx create --use --name mcparr-builder 2>/dev/null || docker buildx use mcparr-builder',
