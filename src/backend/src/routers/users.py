...
from ..models.user_mapping import UserMapping, UserRole, MappingStatus
...
async def get_user_mapping_statistics(db: AsyncSession = Depends(get_db)):
    ...
    active_result = await db.execute(
        select(UserMapping).where(UserMapping.status == MappingStatus.ACTIVE)
    )
    active_mappings = len(active_result.scalars().all())

    pending_result = await db.execute(
        select(UserMapping).where(UserMapping.status == MappingStatus.PENDING)
    )
    ...
    service_stats = {}
    for service_name, service_id in service_mappings:
...
async def create_mappings_from_suggestions(
    ...
    user_mapper = await get_user_mapper()
    results = await user_mapper.create_mappings_from_suggestions(
        db, suggestions, auto_approve_high_confidence
    )
