                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    ...
                    {loadingPreview ? (
                      ...
                      <span className="text-[10px] sm:text-xs px-1 sm:px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 flex-shrink-0">
                        {count}
                      </span>
                    )}
                  </div>
                  <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate">{cat.description}</p>
                </div>
                {isChecked && (
          ...
          <label className="flex flex-col items-center justify-center w-full h-24 sm:h-32 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:border-green-500 dark:hover:border-green-500 transition-colors bg-gray-50 dark:bg-gray-900/50">
            ...
            {importFile ? (
              <div className="text-center px-2">
                ...
                <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{importFileName}</p>
                ...
                {importFile.stats && (
                  <div className="flex flex-wrap gap-1 mt-1.5 sm:mt-2 justify-center">
                    {Object.entries(importFile.stats).map(([key, value]) => (
                      <span key={key} className="text-[10px] sm:text-xs px-1 sm:px-1.5 py-0.5 rounded bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300">
                        {key}: {value as number}
                      </span>
                    ))}
                  </div>
              ...
              {backupCategories.map((cat) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-xs font-medium truncate ${isChecked && hasData ? 'text-green-700 dark:text-green-300' : 'text-gray-600 dark:text-gray-400'}`}>
                      {cat.label}
                    </span>
